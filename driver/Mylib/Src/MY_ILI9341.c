...
static SPI_HandleTypeDef *lcdSPIhandle = SPI1;
//Chip Select pin
...
static void ILI9341_GPIO_Write(GPIO_TypeDef *port, uint16_t pin, uint8_t value)
{
	if(value)
	{
		port->BSRR = (1U << pin);
...
{
	while((lcdSPIhandle->SR & SPI_TXE_FLAG) == 0U)
	{
	}
	while((lcdSPIhandle->SR & SPI_BUSY_FLAG) != 0U)
	{
	}
}
...
static void ILI9341_SPI_EnableTxOnly(void)
{
	...
	lcdSPIhandle->CR1 &= ~(1U << SPI_CR1_SPE);
	lcdSPIhandle->CR1 |= (1U << SPI_CR1_BIDIMODE) | (1U << SPI_CR1_BIDIOE);
	lcdSPIhandle->CR1 |= (1U << SPI_CR1_SPE);
}
...
static void ILI9341_SPI_Write8(uint8_t data)
{
	...
	*((_vo uint8_t *)&lcdSPIhandle->DR) = data;
	ILI9341_SPI_WaitIdle();
}
...
static void ILI9341_SPI_WriteDMA(const uint8_t *data, uint32_t size)
{
	...
	DMA2->S[3].CR = 0;
	DMA2->S[3].PAR = (u32)&lcdSPIhandle->DR;
	...
	lcdSPIhandle->CR2 |= (1U << SPI_CR2_TXDMAEN);
...
static void ILI9341_WriteDataBytes(const uint8_t *data, uint32_t size)
{
...
void ILI9341_SendData_Multi(uint16_t Colordata, uint32_t size)
{
	...
	{
		ili9341_dma_buffer[i] = (uint8_t)(Colordata >> 8);
